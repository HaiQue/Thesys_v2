...
const deleteThesis = async (req, res) => {
  ...
  //checkPermissions(req.user, thesisObj.createdBy);
...
const reserveThesis = async (req, res) => {
  ...
  // update the status of the thesis here
...
const uploadThesisAttachment = async (req, res) => {
  const { id: thesisId } = req.params;
  const { fileName, fileContent } = req.body; // Assuming this data is sent in the request body
