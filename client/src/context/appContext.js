...
const AppProvider = ({ children }) => {
  ...
  // change later. This is possible fix if not remove from other components
  const logoutUser = () => {
  ...
  // change later so that only some roles can do this
  const createThesis = async (professorId) => {
    dispatch({ type: CREATE_THESIS_BEGIN });
    try {
      // change later added role
  ...
  // change later edit so that some roles can do this
